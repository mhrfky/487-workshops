...
class PythonChat :
    ...
    def send_packet(self, target_ip, port, packet):
        try:
            # strr = 'echo ' + packet + ' | nc -q 0 ' + str(target_ip) + ' ' + str(self.port)
            
            # os.system(strr )
            ps = subprocess.Popen(('echo', packet), stdout=subprocess.PIPE)
            output = subprocess.check_output(('nc', '-q', '0' ,target_ip , str(port) ), stdin=ps.stdout)    
            ps.wait()
            # subprocess.call(["nc", packet, target_ip, port])
        except:
    ...
    def receivePacket(self):
        ...
        while(True):
            try:
                process = subprocess.Popen(["nc", "-l", str(self.port)],
                ...
                message = stdout.decode("utf-8").replace("'", '"')[:-1]
